...
`PUT settings` merges supplied fields into current settings. Supported fields include `port`, `modelId`, `maxOutputTokens`, `cpuThreads`, `contextProfile`, `historyLimit`, `authToken`, `toolExposure`, `autoStart` and `currentSessionId`.
