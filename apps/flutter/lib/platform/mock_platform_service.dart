...
class MockPlatformService implements PlatformService {
  ...
  Map<String, dynamic> _sessionDetails(String sessionId) {
    ...
    final match = session.cast<Map>().where((s) => s['id'] == sessionId).toList();
    final id = match.isNotEmpty ? sessionId : (session.isNotEmpty ? session.first['id'] : 'chat_default');
    return {
      'session': {
        'id': id,
        'title': 'Mock session',
        ...
        'messages': [
          {'id': 1, 'role': 'user', 'text': '请先调用 runtime_status，再调用 model_list，然后总结。'},
          {
            'id': 2,
            'role': 'assistant',
            'text': '我先看一下运行状态。',
            'contentJson': {'reasoning': '用户要求先查询运行状态，我应该调用 runtime_status 工具。'},
          },
          {'id': 3, 'role': 'tool', 'name': 'runtime_status', 'text': '{"running": true, "port": 11434}'},
          {'id': 4, 'role': 'assistant', 'text': '运行正常。再看一下模型列表。'},
          {'id': 5, 'role': 'tool', 'name': 'model_list', 'text': '{"models": ["deepseek-v4-flash"]}'},
  ...
  static Map<String, dynamic> _initialState() => {
        ...
        'historyLimit': 64,
        'agentMaxRounds': 32,
        'agentPolicy': {'maxRounds': 32, 'roundsMin': 8, 'roundsMax': 64, 'maxToolCalls': 64},
        'cloudEnabled': true,
        ...
        'requestCount': 12,
        'recentRequests': [
          {'method': 'GET', 'endpoint': '/health', 'status': 200, 'elapsedMs': 3, 'time': 0},
        ],
        'lastError': '',
        ...
        'device': {'interactive': true, 'screenOn': true, 'keyguardLocked': false, 'activityForeground': true},
      };
}
