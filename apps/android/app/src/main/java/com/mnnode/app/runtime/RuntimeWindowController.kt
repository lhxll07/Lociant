...
class RuntimeWindowController private constructor(
    ...
    private fun setError(message: String): JSONObject {
        ...
        eventExecutor.execute {
            runCatching {
                MNNodeRuntime.sessionStore(context).recordRuntimeEvent(
                    sceneId = "runtime",
    ...
    private fun render(root: LinearLayout) {
        val runtime = MNNodeRuntime.runtimeSummary(context)
        root.removeAllViews()
    ...
    private fun savedState(): JSONObject =
        MNNodeRuntime.localStore(context).getObject(SETTINGS_NAMESPACE, SETTINGS_KEY)

    private fun persist(windowParams: WindowManager.LayoutParams) {
        MNNodeRuntime.localStore(context).set(SETTINGS_NAMESPACE, SETTINGS_KEY, JSONObject()
    ...
    private fun isSupported() = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
